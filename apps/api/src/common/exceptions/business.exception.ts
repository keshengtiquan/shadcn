...
export class BusinessException extends HttpException {
  ...
  constructor(code: ResponseCode | number, message?: string, data?: any) {
    const msg =
      message ?? getResponseMessage(code as ResponseCode) ?? "业务处理失败";
...
export class AuthenticationException extends HttpException {
  constructor(
    code: ResponseCode | number = ResponseCode.UNAUTHORIZED,
    message?: string,
  ) {
    const msg =
      message ?? getResponseMessage(code as ResponseCode) ?? "认证失败";
...
export class AuthorizationException extends HttpException {
  constructor(
    code: ResponseCode | number = ResponseCode.FORBIDDEN,
    message?: string,
  ) {
    const msg =
      message ?? getResponseMessage(code as ResponseCode) ?? "权限不足";
...
export class ValidationException extends HttpException {
  constructor(
    ...
    code: ResponseCode | number = ResponseCode.BAD_REQUEST,
  ) {
...
export class NotFoundException extends HttpException {
  constructor(
    resource: string = "资源",
    code: ResponseCode | number = ResponseCode.DATA_NOT_FOUND,
  ) {
    super(
      {
        code,
